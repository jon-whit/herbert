...
OpenCV is an open source computer vision and machine learning software library \cite{OpenCV}. The OpenCV library is leveraged to perform image analysis on each image captured from the Rubik's Cube, which is used to generate a matrix model for any cube orientation.
...
=======
Based on these color ranges, the categorization algorithm can be implemented based the average of all the pixel values in the facelet region or the frequency of each pixel value. A categorizaton algorithm based on the average of HSV values was quick, but it was also error-prone due to outliers. A categorization algorithm based on frequency of each HSV component was robust, but less optimized. Our final implementation computes a histogram of the facelet pixel region using OpenCV. Based on the histogram, the frequency of each of the facelet colors is determined. The highest frequency of a facelet color determines the color that facelet is binned into.
>>>>>>> b56adde0299d2e886e10a9887a0c2f10750b364b
