% pre-amble section
\documentclass[final, letterpaper, 10 pt, conference, onecolumn]{IEEEconf}
...
\begin{figure}[!ht]
\centering
\includegraphics[scale=0.20]{actual_cube_capture.png}
...
The final camera orientation uses a single camera to capture the entire state of the cube as shown in Fig. \ref{fig:Obscured Cube}. A static sequence of rotation using the mechnical system is used to capture multiple 
...
Kcube is a C++ application developed by Greg Schmidt that utilizes Kociemba's two-phase algorithm which uses two stages of an iterative depth first search algorithm \cite{TheTwo-PhaseAlgorithm}. We utilized the Kcube application to generate an suboptimal solution sequence in an configurable time frame needed to solve the Rubik's Cube that was captured during the image processing phase. 
...
In order for a rotation to be successful it must end at a 90 degree angle as to not interrupt the rotation of another arm.  These exact angles are hard to accomplish by counting steps because we are unable to detect whether steps were skipped.  To compensate for this we used ifrared break sensors (see Fig. \ref{fig:Infrared Break Sensor}).  The sensor is placed on the arm as to have the sensor broken at all angles but at 90 degree increments.  These break sensors are controlled through connections on the motor control boards.

\begin{figure}[!ht]
\centering
\includegraphics[scale=0.5]{InfraredBreakSensor.jpg}
...
The original idea behind the six arm solution was to have 6 socket-like arm pieces to actuate in to spin the Rubik's cube.  The first iteration was a simple arm that had a square socket and connects to the stepper motor.  The inner part of the socket had chamfered edges to correct for any error in alignment.  The next iteration included tabs for break sensor detection and it was long to accomodate the length of the center pin.  Also the socket was seperated into four prongs instead of an entire socket as to allow for more visibility of the cube.  For the last iterations we changed from tabbed break sensor detection to slits to allow for more precision on angle detection.  We also shortened the arm and added more chamfered edges for corrections.  We angled out the out the end of the prongs to provide more alignment correction.  Other corrections included slight height changes to fit the slider piece and circular cutouts to avoid arm collisions.  Below is an image showing examples from these iterations (see Fig. \ref{fig:Arm Progression}). 
