...
OpenCV is an open source computer vision and machine learning software library \cite{OpenCV}. The OpenCV library is leveraged to perform image analysis on a Rubik's Cube, which is used to generate a matrix model for any cube orientation. The OpenCV library was installed on the SBC and used to capture and analyze video frames from the USB connected cameras.
...
The pixel region of each of the facelets  must be identified for color recognition. Initially, a combination of grayscale conversion and contour filtering image processing technique is used to dynamically detect a bounding contour around each of the facelets. However, the dynamic approach to identifying a facelet pixel region was inconsistent because of mechanical imprecision. 

Because the camera and the Rubik's Cube remains fixed inside the mechanical frame, a static approach is used in the final implementation. Through a GUI interface, a static polygon for each of the facelets was defined. Each of the static polygons bounds a facelet pixel region. The static approach to identifying the cube orientation proved to be consistent but necessitated high maintenance. Each of the static polygons must be redefined each time the camera position changes or if a mechanical change affects the orientation of the cube. 
...
Lighting condition severally affects the robustness of the color recognition algorithm. Color values on a Rubik's Cube facelet drastically changes if the facelet is reflective or if a shadow is casted upon it. Although maintaining a consistent lighting around the mechnical frame will mitigate a majority of these problems, the color recognition algorithm should remain robust under normal lighting conditions.

Choosing a color space that is robust to lighting conditions was critical to enabling a robust color recognition algorithm. Several color spaces such as RGB, CMYK, and HSV were examined. The HSV color space provided the best consistency under various lighting conditions. The three components of the HSV color space is hue, saturation, and value. The hue component describes the similarity of the color to an unique hue: red, green, blue, and yellow as shown in Fig. \ref{fig:HSV}. The saturation component measures the intensity or colorfulness of the specific hue. Finally, the value measures the brightness of the color value. HSV is ideal for developing a robust color recognition because shadows and high gloss values merely changes a the value component of the color without affecting the hue and saturation component of the color. Additionally, the hue component can be used to distinctly distinguishes each of the primary color such as red, green, and blue that are found on a Rubik's Cube.
...
After identifying the bounding contour around each of the facelet, a categorization algorithm was used to determine the color on the facelet. The image was first converted to the HSV color space. The hue and saturation components of each pixel value is then used to categorize the color. Several color histogram such as the one shown in Fig. \ref{fig:Example Histogram} were constructed for each facelet color and were used to determine the ranges of each HSV component to use to categorize color. For example, the histogram shown in Fig. \ref{fig:Example Histogram} was constructed for a blue facelet and the hue values range between 100 to 130 and the saturation values ranges from 150 to 180. The resulting HSV ranges are shown in Table. \ref{table:color ranges}.

Based on these color ranges, the categorization algorithm can be implemented based the average of all the pixel values in the facelet region or the frequency of each pixel value. A categorizaton algorithm based on the average of HSV values was quick but also error-prone due to outliers. A categorization algorithm based on frequency of each HSV component was robust but less optimized. The final implementation computes a histogram of the facelet pixel region and computes a HSV histogram using OpenCV. Based on the histogram, the frequency of each facelet colors are determined. The highest frequency of a facelet color determine the categorized color.

Initially the algorithm implemented needed approximately a minute to categorize each facelet because of the unoptimized looping in Python and hardware limitations. After several iterative optimization using the OpenCV library, the overall color categorization time used approximately three seconds.  In the future, the categorization can still be optimized for both robustness and speed. For example, the image resolution or the overall pixel region can be reduced to improve performance at the cost of reliability. Additionally, a filtering algorithm can be used to reduce the effects on abnormal lighting. 
...
\subsection{Serial Communication and Command}
After retrieving the solution sequence from Kcube, the solution sequence is sent over to the FPGA component serially. A serial python communication library with CRC32 error detection is implemented and used communicate to the FPGA. The command sent over to the FPGA is the ExecuteMoves command that described in section \ref{sec:Firmware}.
