...
CXHeaderCtrl::CXHeaderCtrl()
{
    ...
    m_crHeaderText  = ::GetSysColor(COLOR_INFOTEXT);
}
...
int CXHeaderCtrl::DrawText(CDC* pDC, CRect rect, LPHDITEM lphdi)
{
    ...
    if (rect.Width() > 0 && lphdi->mask & HDI_TEXT && lphdi->fmt & HDF_STRING)
	{
        ...
        // Get text rect with DT_CALCRECT
        pDC->DrawText(lphdi->pszText, -1, textRect,
            DT_CENTER|DT_END_ELLIPSIS|DT_SINGLELINE|DT_VCENTER|DT_CALCRECT);

        textRect.OffsetRect((rect.Width() - textRect.Width()) / 2, (rect.Height() - textRect.Height()) / 2);
        ...
        else
        {
            ...
            pDC->SetTextColor(m_cr3DHighLight);
            pDC->DrawText(lphdi->pszText, -1, textRect,
                DT_END_ELLIPSIS|DT_SINGLELINE);
            textRect.OffsetRect(-1, -1);

            //Draw disabled text
            pDC->SetTextColor(m_cr3DShadow);
            pDC->DrawText(lphdi->pszText, -1, textRect,
                DT_END_ELLIPSIS|DT_SINGLELINE);
        }
    }
