...
public:
    ...
    struct MenuItem
    {
        ...
        MenuItem() : bitmap(0) {};
        MenuItem(CString label, int bitmap) : label(label), bitmap(bitmap) {};
    };
...
public:
	...
	DWORD SetSplit(BOOL bSplit = TRUE, BOOL bRepaint = TRUE);
	DWORD SetSplitWidth(int nSplitWidth = BTNST_SPLIT_WIDTH, BOOL bRepaint = TRUE);

    DWORD DrawDefault(BOOL bDrawDefault = TRUE, BOOL bRepaint = TRUE);

    DWORD DrawBorder(BOOL bDrawBorder = TRUE, BOOL bRepaint = TRUE);
    DWORD DrawFocusRect(BOOL bDrawSelectedBorder = TRUE, BOOL bRepaint = TRUE);
    ...
    void SetNumberedStylePopupMenu(BOOL numbered = true);
    void SetXPStylePopupMenuStyle(BOOL xpStyle = true);
    BOOL InsertMenuItem(CString label, int bitmap, int pos);
    BOOL RemoveMenuItem(int pos = 0, int count = 1);
    int  GetMenuItemCount();
#else
	DWORD SetMenu(UINT nMenu, HWND hParentWnd, BOOL bRepaint = TRUE);
    DWORD SetMenu(CMenu* nMenu, HWND hParentWnd, BOOL bRepaint = TRUE);
#endif
...
protected:
    ...
    BOOL        m_bShowFocus;       // Draw focus border?
	BOOL		m_bIsDisabled;		// Is button disabled?
	BOOL		m_bIsDefault;		// Is default button?
    BOOL        m_bDrawDefault;     // Draw default border?
    BOOL		m_bIsCheckBox;		// Is the button a checkbox?
    BOOL        m_bIsSplitButton;   // Is split (menu) button?
    ...
    int         m_nSplitBtnWidth;
#ifdef	BTNST_USE_BCMENU
    BCMenu		m_menuPopup;		// BCMenu class instance
    CMenu       m_subMenu;
    BOOL        m_bMenuNumbered;
    BOOL        m_bMenuXpStyle;
    CArray<MenuItem,MenuItem&> m_menuItems;
#else
