...
DWORD CSerialEx::ThreadProc (void)
{
	...
	do
	{
        ...
        LONG status = WaitEvent(&ovInternal);
        if (status != ERROR_SUCCESS)
        {
			_RPTF1(_CRT_WARN,"CSerialEx::ThreadProc - WaitEvent failed (status = %d)\n", status);
        }

		// Wait for the overlapped operation to complete
		status = ::WaitForSingleObject(m_hevtOverlappedWorkerThread,INFINITE);
        if (status != WAIT_OBJECT_0)
		{
			...
			_RPTF2(_CRT_WARN,"CSerialEx::ThreadProc - Unable to wait until COM event has arrived (status = %d, lastError = %d)\n", status, m_lLastError);
			return m_lLastError;
		}
#else
