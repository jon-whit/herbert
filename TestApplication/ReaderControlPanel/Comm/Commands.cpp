...
Commands::PcbManual::PcbManual(const CString& cmd, BOOL addHeader)
    : Manual(HEADER_TO_PRB_FROM_SBC, cmd, addHeader)
{
}
...
Commands::GetPcbStatus::GetPcbStatus()
    : Transaction(HEADER_TO_PRB_FROM_SBC)
    , m_powerSupply3_3VError(FALSE)
    , m_powerSupply12VError(FALSE)
    , m_powerSupply24VError(FALSE)
    , m_sensorPowerError(FALSE)
    , m_pwrDownRequested(FALSE)
{
    m_cmdPkt.SetCmd(GET_PCB_GET_STATUS_STR);
}



BOOL Commands::GetPcbStatus::processRsp(CommPkt* rspPkt)
{
    if(rspPkt->GetDataLen() < 2)
    {
        return FALSE;
    ...
    if(rspPkt->GetData(1) == "PwrDown")
    {
        m_pwrDownRequested = true;
    }
    else if(rspPkt->GetData(1) == "None")
    {
        m_pwrDownRequested = false;
    }
    else
    ...
    while(++index < rspPkt->GetDataLen())
    {
        CString error = rspPkt->GetData(index);

        int     delimiterPos = error.Find(':');
        CString component;
        CString voltage;

        if(delimiterPos >= 0)
        {
            component = error.Left(delimiterPos);
            voltage   = error.Mid(delimiterPos + 1);
        }
        else
        {
            component = error;
        ...
        if(component == "Power3.3V")
        {
            m_powerSupply3_3VVoltage = voltage;
            m_powerSupply3_3VError   = true;
        }
        else if(component == "Power12V")
        {
            m_powerSupply12VVoltage = voltage;
            m_powerSupply12VError   = true;
        }
        else if(component == "Power24V")
        {
            m_powerSupply24VVoltage = voltage;
            m_powerSupply24VError   = true;
        }
        else if(component == "Sensor")
        {
            m_sensorPowerVoltage = voltage;
            m_sensorPowerError   = true;
        }
        else
        {
            return false;
    ...
    if(index < rspPkt->GetDataLen())
    {
        // components without voltages
        return false;
    }
...
Commands::GetVoltage::GetVoltage(const CString& device)
    : Transaction(HEADER_TO_PRB_FROM_SBC)
{
    m_cmdPkt.SetCmd(GET_PCB_GET_VOLTAGE_STR);
    m_cmdPkt.AddData(device);
}

BOOL Commands::GetVoltage::processRsp(CommPkt* rspPkt)
{
    if(rspPkt->GetDataLen() != 2)
    {
        return FALSE;
...
Commands::GetOnState::GetOnState(const CString& device)
    : Transaction(HEADER_TO_PRB_FROM_SBC)
{
    m_cmdPkt.SetCmd(GET_PCB_GET_ON_STATE_STR);
    m_cmdPkt.AddData(device);
}

BOOL Commands::GetOnState::processRsp(CommPkt* rspPkt)
{
    if(rspPkt->GetDataLen() != 2)
    {
        return FALSE;
    ...
    if(rspPkt->GetData(1) == "On")
    {
        m_on = true;
    }
    else if(rspPkt->GetData(1) == "Off")
    {
        m_on = false;
    }
    else
...
Commands::SetOnState::SetOnState(const CString& device, BOOL on)
    : Transaction(HEADER_TO_PRB_FROM_SBC), m_on(on)
{
    m_cmdPkt.SetCmd(GET_PCB_SET_ON_STATE_STR);
    m_cmdPkt.AddData(device);
    m_cmdPkt.AddData(on ? "On" : "Off");
}

BOOL Commands::SetOnState::processRsp(CommPkt* rspPkt)
{
    if(rspPkt->GetDataLen() != 1)
    {
        return FALSE;
