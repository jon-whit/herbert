...
CSerial::CSerial ()
	...
	, m_hReadEventOverlapped(0)
	, m_hWriteEventOverlapped(0)
	, m_hWaitEventOverlapped(0)
#endif
{
}
...
LONG CSerial::Open (LPCTSTR lpszDevice, DWORD dwInQueue, DWORD dwOutQueue, bool fOverlapped)
{
	...
	_ASSERTE(m_hReadEventOverlapped == 0);
	_ASSERTE(m_hWriteEventOverlapped == 0);
	_ASSERTE(m_hWaitEventOverlapped == 0);

	// Create the event handle for internal overlapped operations (manual reset)
	if (fOverlapped)
	{
		m_hReadEventOverlapped  = ::CreateEvent(0,true,false,0);
		m_hWriteEventOverlapped = ::CreateEvent(0,true,false,0);
		m_hWaitEventOverlapped  = ::CreateEvent(0,true,false,0);
		if (m_hReadEventOverlapped  == 0 ||
			m_hWriteEventOverlapped == 0 ||
			m_hWaitEventOverlapped  == 0)
		{
            ...
            // Free event handles
			if (m_hReadEventOverlapped)
			{
				::CloseHandle(m_hReadEventOverlapped);
                m_hReadEventOverlapped = 0;
            }

			if (m_hWriteEventOverlapped)
			{
				::CloseHandle(m_hWriteEventOverlapped);
                m_hWriteEventOverlapped = 0;
            }

			if (m_hWaitEventOverlapped)
			{
				::CloseHandle(m_hWaitEventOverlapped);
                m_hWaitEventOverlapped = 0;
            }
...
LONG CSerial::WaitEvent (LPOVERLAPPED lpOverlapped, DWORD dwTimeout)
{
	...
	// Check if an overlapped structure has been specified
    if (!m_hWaitEventOverlapped && (lpOverlapped || (dwTimeout != INFINITE)))
	{
		// Set the internal error code
	...
	if (!lpOverlapped && m_hWaitEventOverlapped)
	{
		// Setup our own overlapped structure
		memset(&ovInternal,0,sizeof(ovInternal));
		ovInternal.hEvent = m_hWaitEventOverlapped;
	...
	_ASSERTE(!m_hWaitEventOverlapped || HasOverlappedIoCompleted(lpOverlapped));
...
LONG CSerial::SetupHandshaking (EHandshake eHandshake)
{
	...
	case EHandshakeOffWithPower:
		dcb.fOutxCtsFlow = false;					// Disable CTS monitoring
		dcb.fOutxDsrFlow = false;					// Disable DSR monitoring
		dcb.fDtrControl = DTR_CONTROL_ENABLE;		// Enable DTR for line power
		dcb.fOutX = false;							// Disable XON/XOFF for transmission
		dcb.fInX = false;							// Disable XON/XOFF for receiving
		dcb.fRtsControl = RTS_CONTROL_ENABLE;		// Enable RTS for line power
		break;
...
LONG CSerial::Write (const void* pData, size_t iLen, DWORD* pdwWritten, LPOVERLAPPED lpOverlapped, DWORD dwTimeout)
{
	...
	*pdwWritten = 0;
	...
	// Check if an overlapped structure has been specified
	if (!m_hWriteEventOverlapped && (lpOverlapped || (dwTimeout != INFINITE)))
	{
		// Set the internal error code
		m_lLastError = ERROR_INVALID_FUNCTION;
	...
	// Wait for the event to happen
	OVERLAPPED ovInternal;
	if (!lpOverlapped && m_hWriteEventOverlapped)
	{
		// Setup our own overlapped structure
		memset(&ovInternal,0,sizeof(ovInternal));
		ovInternal.hEvent = m_hWriteEventOverlapped;
	...
	// Make sure the overlapped structure isn't busy
	_ASSERTE(!m_hWriteEventOverlapped || HasOverlappedIoCompleted(lpOverlapped));
		...
		if (lpOverlapped == &ovInternal)
		{
			...
			case WAIT_OBJECT_0:
				...
				if (!::GetOverlappedResult(m_hFile,lpOverlapped,pdwWritten,FALSE))
				{
                    ...
                    CString msg;
                    msg.Format("CSerial::Write - Overlapped completed without result (error: %d)\n", m_lLastError);
                    _RPTF0(_CRT_WARN,msg);
					return m_lLastError;
				}
				break;
...
LONG CSerial::Read (void* pData, size_t iLen, DWORD* pdwRead, LPOVERLAPPED lpOverlapped, DWORD dwTimeout)
{
	...
	*pdwRead = 0;
	...
	// Check if an overlapped structure has been specified
	if (!m_hReadEventOverlapped && (lpOverlapped || (dwTimeout != INFINITE)))
	{
		// Set the internal error code
		m_lLastError = ERROR_INVALID_FUNCTION;
	...
	if (lpOverlapped == 0)
	{
		// Setup our own overlapped structure
		memset(&ovInternal,0,sizeof(ovInternal));
		ovInternal.hEvent = m_hReadEventOverlapped;
	...
	// Make sure the overlapped structure isn't busy
	_ASSERTE(!m_hReadEventOverlapped || HasOverlappedIoCompleted(lpOverlapped));
	...
	if (!::ReadFile(m_hFile,pData,iLen,pdwRead,lpOverlapped))
	{
		...
		if (lLastError != ERROR_IO_PENDING)
		{
			...
			// Issue an error and quit
            CString msg;
            msg.Format("CSerial::Read - Unable to read the data (error: %d)\n", m_lLastError);
            _RPTF0(_CRT_WARN,msg);
			return m_lLastError;
		}
