...
void CXListCtrl::OnPaint()
{
	...
	if (GetItemCount() <= 0)
	{
		...
		COLORREF crText  = IsWindowEnabled() ? m_crWindowText : m_cr3DShadow;
		COLORREF crBkgnd = m_crWindow;
...
//
// Get / Set default text (shown when no items are shown)
//
CString CXListCtrl::GetDefaultText()
{
    return m_strDefaultText;
}

void CXListCtrl::SetDefaultText(const CString& defaultText)
{
    m_strDefaultText = defaultText;
}
...
{
    int nCurrentItem = GetCurSel();
    if(nCurrentItem >= 0)
        SetItemState(nCurrentItem, 0, LVIS_FOCUSED | LVIS_SELECTED);
