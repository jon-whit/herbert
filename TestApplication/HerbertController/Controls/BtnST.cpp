...
//#define BS_TYPEMASK SS_TYPEMASK		// These should be defined by the system, but are not
//#define TPM_HORPOSANIMATION 0		// These should be defined by the system, but are not
//#define TPM_VERPOSANIMATION 0		// These should be defined by the system, but are not
...
CButtonST::CButtonST()
{
    ...
    m_bMenuNumbered     = FALSE;
    m_bMenuXpStyle      = FALSE;
    ...
    // By default draw focus border
    m_bShowFocus = FALSE; //TRUE;
	...
	m_bIsDefault = FALSE;
    // By default show the default/focus frame
    m_bDrawDefault = TRUE;
    ...
    // By default the button is not a split button
    m_bIsSplitButton = FALSE;
    ...
    m_nSplitBtnWidth      = BTNST_SPLIT_WIDTH;
	...
	else
	{
		...
		if (!m_bIsSplitButton)
		{
            if(m_menuPopup.m_hMenu)
            {
			    ...
			    if (dwRetValue && m_csCallbacks.hWnd)	::SendMessage(m_csCallbacks.hWnd, m_csCallbacks.nMessage, dwRetValue, m_csCallbacks.lParam);
            ...
            else if(m_menuItems.GetSize())
            {
                ...
                if(m_bMenuXpStyle)
                {
                    ...
                    menuPopup.SetMenuDrawMode(m_bMenuXpStyle);
                    ...
                    for(int i = 0; i < m_menuItems.GetSize(); ++i)
                    {
                        ...
                        if(m_bMenuNumbered)
		                {
			                menuLine.Format("%d\t  %s", i + 1, m_menuItems[i].label);
		                }
		                else
		                {
			                menuLine.Format("%s", m_menuItems[i].label);
		                }

                        menuPopup.AppendMenu(MF_STRING, i + 1, menuLine);

                        if(m_menuItems[i].bitmap)
                        {
                            menuPopup.ModifyODMenuA(NULL, i + 1, m_menuItems[i].bitmap);
                        }
                    }

                    dwRetValue = menuPopup.TrackPopupMenu(TPM_LEFTALIGN | TPM_LEFTBUTTON | TPM_RIGHTBUTTON | TPM_NONOTIFY | TPM_RETURNCMD | TPM_HORPOSANIMATION | TPM_VERPOSANIMATION,
                                                            rc.right - m_nSplitBtnWidth, rc.bottom, this, NULL);
                }
                else
                {
                    CMenu menuPopup;
                ...
                if (dwRetValue && m_csCallbacks.hWnd)
                {
                    ::SendMessage(m_csCallbacks.hWnd, m_csCallbacks.nMessage, dwRetValue - 1, m_csCallbacks.lParam);
                }
            }
		} // if
		else
...
void CButtonST::DrawItem(LPDRAWITEMSTRUCT lpDIS)
{
    ...
    if(m_bIsSplitButton)
    {
        itemRect.right    -= m_nSplitBtnWidth;
        captionRect.right -= m_nSplitBtnWidth;
    }
	...
	{
		// Draw the focus rect
		if (m_bIsFocused && m_bShowFocus && !m_bIsSplitPressed)
		{
    ...
    if(m_bIsSplitButton)
    {
	    ...
	    // Drop down split                    //
	    ////////////////////////////////////////
	    splitRect.DeflateRect(1, 0, 1, 1);
	    splitRect.left = splitRect.right - m_nSplitBtnWidth;

        if(m_bIsFocused)// && m_bShowFocus)
        {
            ++splitRect.right;
        }
        ...
        if(!m_bIsFlat || m_bMouseOnButton)
        {
            if (!m_bIsSplitPressed)
            {
	            ...
	            pDC->MoveTo(splitRect.left, splitRect.top);
	            pDC->LineTo(splitRect.left, splitRect.bottom);


	            CPen brLite(PS_SOLID,1,GetSysColor(COLOR_3DHILIGHT));
	            pDC->SelectObject(&brLite);
	            pDC->MoveTo(splitRect.left+1, splitRect.top);
	            pDC->LineTo(splitRect.left+1, splitRect.bottom);
	    ...
	    CPoint pt(splitRect.CenterPoint());
        pt.Offset(0, 2); // Shift Arrow down.
        pt += CPoint(m_bIsPressed + m_bIsSplitPressed, m_bIsPressed + m_bIsSplitPressed);


        if(m_bIsDisabled || (!m_menuPopup.m_hMenu && !m_menuItems.GetSize()))
        {
            CPen penHighlight(PS_SOLID, 1, ::GetSysColor(COLOR_3DHILIGHT));
        ...
        {
            CPen penBlack(PS_SOLID, 1, GetSysColor(COLOR_WINDOWTEXT));
...
//
////////////////////////////////////////////////////////////////////////////////
void CButtonST::DrawArrow(CDC* pDC,CPoint ArrowTip)
{
	CPoint ptDest;
...
DWORD CButtonST::SetSplit(BOOL bSplit, BOOL bRepaint)
{
	m_bIsSplitButton = bSplit;
	if (bRepaint)	Invalidate();
...
DWORD CButtonST::SetSplitWidth(int nSplitWidth, BOOL bRepaint)
{
	m_nSplitBtnWidth = nSplitWidth;
	if (bRepaint)	Invalidate();
...
DWORD CButtonST::DrawDefault(BOOL bDrawDefault, BOOL bRepaint)
{
	m_bDrawDefault = bDrawDefault;
	// Repaint the button
	if (bRepaint) Invalidate();
...
// This function sets if the button selected (focus) border must be drawn.
//
// Parameters:
//		[IN]	bShowFocus
//				If TRUE the selected (focus) border will be drawn.
//		[IN]	bRepaint
//				If TRUE the control will be repainted.
//
// Return value:
...
//			Function executed successfully.
//
DWORD CButtonST::DrawFocusRect(BOOL bShowFocus, BOOL bRepaint)
{
...
DWORD CButtonST::SetMenu(CMenu* nMenu, HWND hParentWnd, BOOL bRepaint)
{
	HINSTANCE	hInstResource	= NULL;
...
void CButtonST::SetNumberedStylePopupMenu(BOOL numbered)
{
    m_bMenuNumbered = numbered;
}

void CButtonST::SetXPStylePopupMenuStyle(BOOL xpStyle)
{
    m_bMenuXpStyle = xpStyle;
}


BOOL CButtonST::InsertMenuItem(CString label, int bitmap, int pos)
{
    MenuItem newItem(label, bitmap);
    
    if(pos < m_menuItems.GetSize())
    {
        m_menuItems.InsertAt(pos, newItem);
    }
    else
    {
...
BOOL CButtonST::RemoveMenuItem(int pos, int count)
{
    if(count < 1 || (pos + count) > m_menuItems.GetSize())
    {
        return false;
    }

    m_menuItems.RemoveAt(pos, count);
...
int CButtonST::GetMenuItemCount()
{
...
DWORD CButtonST::OnDrawBackground(CDC* pDC, CRect* pRect)
{
	...
	if (m_bIsFlat == FALSE)
	{
		if (m_bDrawDefault && (m_bIsFocused || m_bIsDefault))
		{
			CBrush br(RGB(0,0,0));  
...
DWORD CButtonST::OnDrawBorder(CDC* pDC, CRect* pRect)
{
    ...
    else if(m_bIsSplitPressed)
    {
        CRect splitRect = *pRect;

        splitRect.left = splitRect.right - m_nSplitBtnWidth;
        pRect->right -= (m_nSplitBtnWidth - 1);

		if (m_bIsFlat)
		{
			if (m_bDrawBorder)
            {
				pDC->Draw3dRect(splitRect, ::GetSysColor(COLOR_BTNSHADOW), ::GetSysColor(COLOR_BTNHILIGHT));
            }
		}
		else    
		{
			CBrush brBtnShadow(GetSysColor(COLOR_BTNSHADOW));
			pDC->FrameRect(splitRect, &brBtnShadow);
		}
    }
	...
	if (m_bIsFlat)
	{
		if ((m_bMouseOnButton || m_bIsSplitPressed) && m_bDrawBorder)
        {
...
void CButtonST::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (m_bIsSplitButton && HitMenuBtn(point))
	{
        ...
        if(m_bMenuXpStyle)
        {
            ...
            menuPopup.AppendMenu(MF_STRING, 1, "", 1);
            menuPopup.RemoveMenu(1, 0);//  .AppendMenu(0);
            ...
            for(int i = 0; i < m_menuItems.GetSize(); ++i)
            {
                ...
                menuPopup.AppendMenuA(MF_STRING, i + 1, menuLine, m_menuItems[i].bitmap);
...
BOOL CButtonST::HitMenuBtn(CPoint point)
{
	if (!m_menuPopup.m_hMenu && !m_menuItems.GetSize())
	...
	CRect rect;
	GetWindowRect(rect);
	rect.left = rect.right - m_nSplitBtnWidth;
