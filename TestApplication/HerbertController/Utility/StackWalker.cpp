...
#define OUTPUT_MODULE_INFO  0
#define OUTPUT_SYSINIT_INFO 0
#define OUTPUT_OS_VERSION   0
#define OUTPUT_DBG_HELP_ERR 0
...
std::string StackWalker::GetNameFromAddr(DWORD64 pc)
{
    if (m_modulesLoaded == FALSE)
    ...
    if(m_sw->pSGSFA(m_hProcess, pc, &offsetFromSymbol, pSym) != FALSE)
    {
        if(m_sw->pSGLFA(m_hProcess, pc, &offsetFromLine, &line) != FALSE)
        {
            name << line.FileName << " (" << line.LineNumber << "): " << pSym->Name;
        }
    }
...
//C:\Development\Projects\PlateCycler\Beta\Software\Common\Utility\StackWalker.cpp (960): StackWalker::ShowCallstack
...
void StackWalker::OnCallstackEntry(CallstackEntryType eType, CallstackEntry &entry)
{
  ...
  if ( (eType != lastEntry) && (entry.offset != 0) )
  {
    ...
    if (entry.lineFileName[0] == 0)
    {
      ...
      if (entry.offsetFromSymbol == 0)
        _snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "%s: (%s):(0x%p) %s\n", entry.lineFileName, entry.moduleName, (LPVOID) entry.offset, entry.name);
      else
        _snprintf_s(buffer, STACKWALK_MAX_NAMELEN, "%s: (%s):(0x%p) %s + 0x%X\n", entry.lineFileName, entry.moduleName, (LPVOID) entry.offset, entry.name, entry.offsetFromSymbol);
    }
    else
